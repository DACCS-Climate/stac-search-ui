...
def build(build_directory, map_default_latitude, map_default_longitude, map_default_zoom, clean=True):
    if clean:
    ...
    for template in env.list_templates(filter_func=filter_site_templates):
        ...
        with open(build_destination, "w") as f:
            f.write(env.get_template(template).render(map_default_lat = map_default_latitude,
...
if __name__ == "__main__":
    ...
    build(args.build_directory, args.map_default_latitude, args.map_default_longitude, args.map_default_zoom, args.clean)